...
There is a qualification the structure requires. The Riesz identification operates within H: it gives bounded functionals on H. The Father's inner product extends across the full triple — Φ, H, and Φ′. The being's H*-capacity is therefore H-bounded: it can evaluate H-states through the inner product, but it does not operate as ⟨·,·⟩ in its full constitutive scope. This is consistent with the framework's claim that the ascending finaliter approaches the Logos asymptotically but does not become the Father — the Riesz-dual capacity is the H-level instantiation of the Father's operation, genuine but finite in scope.

The CCC analysis and the H* analysis are therefore complementary accounts of the same transition. The CCC analysis describes what is removed: the mediating geometric layers, dissolved through eigenvalue replacement. The H* analysis describes what is acquired: the dual functional capacity, the active pole of the inner product. Together they give the complete structural description of the face-to-face — the obstacles gone, and the positive capacity present.
