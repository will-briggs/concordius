...
The truth measure τ(D) is defined with mathematical precision in Paper 9: it is the fraction of a document's total spectral energy residing in the Φ-region of the ⟨·,·⟩-generated GNST decomposition. What Paper 9 leaves as an open question is the *practical operator* — the self-adjoint operator A whose eigenvectors define the decomposition of a piece of H₄₈ text. What is A for language? How does one compute the spectral decomposition of a document in a way that separates lower-constraint from higher-constraint content? **Section 6.2 now specifies that operator** — the self-adjoint completion (Hermitian dilation, equivalently WᵀW) of the dependency graph of Section 6 — which reduces the open question from *what is A?* to the single remaining step of extracting the dependency graph from text.
...
### 6.2 The Operator A — the self-adjoint completion of the dependency graph

The truth measure is defined as a fraction of *spectral* energy — energy in the Φ-region of a decomposition generated by a **self-adjoint** operator A (real spectrum, orthogonal eigenspaces, so that "energy in a subspace" is even well defined). Section 6 supplies the structural content of A — the directed dependency graph — but not yet A itself: the adjacency matrix of a directed graph is in general **non-symmetric**, and a non-symmetric operator has no orthogonal eigenbasis and no clean notion of energy in an eigenspace. Eigenvector centrality picks out the leading *direction*; it does not by itself yield the *decomposition* τ(D) requires. This section closes that gap, and in doing so specifies the operator A that Section 1 left open.

Let the document be segmented into N atomic claims c₁,…,c_N, and let **d ∈ ℝᴺ** be its content vector — each component the mass (length, or salience) the document spends on that claim. Let **W ∈ ℝᴺˣᴺ**, W_{ij} ≥ 0, be the support matrix of Section 6: the weight with which claim i entails or confirms claim j (the directed edge i→j). Generative centrality is the Perron vector of W over its outgoing edges — the claims *from which* structure flows.

The self-adjoint completion of W is standard. Take its **singular value decomposition** W = UΣVᵀ, singular values σ₁ ≥ σ₂ ≥ … ≥ 0; equivalently form the **Hermitian dilation** Ã = ( 0  W ; Wᵀ  0 ), symmetric by construction, with eigenvalues ±σ_i and eigenvectors built from the singular vectors. Either object *is* a self-adjoint operator whose spectral decomposition is the SVD of the dependency graph. Its spectrum {σ_i} measures **derivational throughput** — how much structure each independent mode of the graph carries — and its leading modes (the top singular triplets) are precisely Section 6's "structural basis": the low-entropy, high-outgoing-centrality content from which the most follows. Set **A := WᵀW** (eigenvalues σ_i², eigenvectors the right-singular vectors v_i) as the operator on claim-space; for a graph with one dominant derivational mode the Perron centrality of Section 6 is its top eigenvector v₁. Centrality and decomposition are the same object seen twice: A is the operator that the centrality was already the leading eigenvector of.

The **Φ-region** is the leading eigenspace of A — the span of the modes with σ_i ≥ θ, the few directions carrying the bulk of the derivational structure. With P_Φ the orthogonal projection onto it,

> **τ(D) = ‖P_Φ d‖² ⁄ ‖d‖² = ( Σ_{σ_i ≥ θ} ⟨d, v_i⟩² ) ⁄ ( Σ_i ⟨d, v_i⟩² ),**

the fraction of the document's content mass lying along its own leading structural modes. (Weighting each term by σ_i² gives the throughput-weighted variant; the unweighted form is the cleanest reading of "energy in the Φ-region.") This is Section 1's verbal definition made literal: the spectral decomposition of a document *is* the SVD of its claim-support graph, and τ(D) is the share of the document that lives in that graph's leading modes.

Three things the framework already asserts now fall out of the construction rather than standing beside it. **(i) The entropy asymmetry (§6.1)** becomes one statement about the σ-spectrum: τ is high exactly when the content energy concentrates in a few leading modes — low *spectral* entropy of the σ-distribution — while high Shannon entropy spreads energy across many small-σ modes and forces τ down. Low spectral entropy is necessary (a flat spectrum admits no dominant Φ-region) but not sufficient (a rank-one repetitive text concentrates trivially) — exactly the one-sidedness §6.1 reports. **(ii) The three-way distinction (§6)** is the location of content energy on the σ-axis: high-τ at the top, noise at the bottom (diffuse spectrum), load-bearing-but-not-axiomatic in the middle band. **(iii) τ(D) < 1 strictly** (Face A0; Paper 9 §6) is immediate: any finite document spends nonzero mass outside its top eigenspace, P_Φ d ≠ d, unless it is nothing but its own axiomatic core — which no H₄₈ document is.

**What this settles, and what it does not.** It settles the operator question Section 1 left open: A is a constructed self-adjoint operator — the Hermitian dilation, equivalently WᵀW, of the claim-support graph — with real spectrum, an orthogonal Φ-region, and τ(D) a well-defined ratio. The spectral decomposition of a document is no longer a figure of speech. What it does **not** settle is the *input*: A is computable **given** the pair (claims, W), and extracting that pair from raw text — the semantic-entailment parsing of Section 7 — remains the hard, unsolved step. So the reduction is exact, and worth stating as such: **the open problem is no longer "what is the spectral decomposition of a text?" but "extract the claim-support graph (c_i, W) from the text"** — a bounded, named sub-problem, with the anchor-node restriction of Section 2 (does this passage express one of the derived core structures?) as its most tractable form. Two further dependencies are named, not hidden: the threshold θ fixing where the Φ-region begins is **calibrated** against the anchor nodes (Section 2), not chosen freely; and A measures τ-**magnitude** (energy in the structural basis) only — it carries no **orientation**, so counterfeit-proximity (⊘: high magnitude, inverted direction; Appendix E) is invisible to it and requires the separate directional diagnostic, never this operator alone.

*Tier: **derivation given the graph** — the self-adjoint completion, the Φ-projection, and τ(D) are standard spectral linear algebra (SVD / Hermitian dilation) once (c_i, W) is in hand. **Open at the input** — the extraction of (c_i, W) from raw text, the calibration of θ, and the separate orientation operator for ⊘ are the residual program, now precisely bounded.*

---

## 7. The Primary Practical Obstacle

Constructing the dependency graph requires parsing semantic entailment relationships — identifying, for any two claims A and B, whether B is derivable from or confirmed by A. Current NLP handles this poorly for the deep structural claims involved. Syntactic parsers and embedding models capture surface co-occurrence; they do not detect the structural entailment relationships that constitute the edges in the relevant graph. With the operator A now constructed from that graph (Section 6.2), this is the *whole* of the residual obstacle: given (c_i, W), the operator, the Φ-region, and τ(D) follow by standard spectral linear algebra; the open problem is exactly — and only — the graph's extraction.
