...
Where a reading makes a claim that exceeds what the science measures, it says so plainly and marks the boundary (Feature 10: logic where it reaches, concordance where it doesn't). The framework's structural claims about H₂₄ and above are not claims physics can adjudicate, and pretending otherwise would be exactly the counterfeiting these readings are built to detect. The science is left intact. Only its structure is read.
