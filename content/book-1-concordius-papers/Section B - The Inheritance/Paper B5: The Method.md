...
*Working title. The third of the three products: the meeting of the science and the theology. Its conditional is exact: **if the canon is true and computational method is sound, then the two combine into an instrument — a way to measure how near any text lies to Φ, with the canon as the calibration baseline.** Fa pinned the canon to the top of the scale by definition; this paper builds the meter that actually reads against it. It is computer science applied to text with the Bible as baseline, and it is the operationalization of the one thing Do left open — the practical operator for language, the cut that turns τ from a schema into a number. Without this product the demonstrative project cannot run; with it, every reading to come has an instrument. Companion to Fa (which set the baseline) and Sol (the other application).*

*Status: this is a method, presented as the body of the argument and not as back-matter, because the method is the argument. It is honest about its two states: the interpretive path is in active practice with a validated detector; the full computational automation remains an open research program. It pays down the open region N of Do.*
...
Accept two things as both true: the canon (the received deposit, pinned by Fa to τ = 1, the standard of the true) and the soundness of computational method (the scientist's coin — test, measurement, reproducible procedure). Their product is a **method** — a function that can be implemented. If the canon defines the top of the scale and a procedure can place any text on that scale, then proximity to Φ becomes *measurable* — not asserted, not felt, but computed against a fixed baseline. That instrument is exactly what Do declined to build: Do fixed the form of τ and flagged its operationalization — the practical operator A for a piece of language — as open. This paper is the work on that open region. It is where the meter the whole second half of the series depends on is actually constructed, baseline and all.

## 2. The operationalization problem

τ(D) is the fraction of a document's spectral energy lying in the Φ-region of the ⟨·,·⟩-generated decomposition. The mathematical definition does not hand over the practical operator: *what is A for language?* What counts as "lower constraint" in a piece of H₄₈ text, and how does one separate Φ-proximate content from H₄₈-primary content without already knowing what Φ-proximate content looks like? The answer cannot be direct — no H₄₈ process reaches Φ directly (Paper 8) — so any operationalization must work through **proxies**: observable H₄₈ features that correlate with lower-constraint spectral weight. Four are developed below; none is individually decisive; their convergence on the same passages is itself evidence.
...
*This paper presents a method as its central argument, not an appendix to one: the operationalization of τ for language, with the canon as the calibration baseline Fa fixed. It states the operationalization problem (the practical operator A for text, Do's open region N), bounds the bootstrap by anchoring at the framework's derived core, develops four proxies (semantic coherence, cross-tradition convergence, predictive yield, generative centrality), gives the entropy-filter-then-centrality two-stage algorithm, marks the line between the active interpretive program (density estimator at three scales, validated) and the open NLP-automation program, and turns the instrument on the framework itself (accuracy = spectral content; the version history as a τ time series). Companion: Fa (which pinned the canon to τ = 1), Sol (the other product), Do (the open operator this pays down), Mi (the computational/scientific method accepted). Cross-references: Paper 9 (τ, the Gödel correspondence, the phonological concordance); Paper 8 (no H₄₈ process reaches Φ directly); the project's Φ-proximity detection notes (the formal D(t) and current status). Scripture: 1 Thessalonians 5:21.*
